...
export default function Home() {
  ...
  const { data: analytics } = trpc.growth.analytics.useQuery(undefined, { enabled: !!user });
  const qualify = trpc.growth.qualifyLead.useMutation();
  const [query, setQuery] = useState("");
  const [showAdd, setShowAdd] = useState(false);
  const [showAi, setShowAi] = useState(false);
  const [transcript, setTranscript] = useState("They want to lose 30 lbs and keep it off, have tried keto and CrossFit before without lasting results, and are ready to invest in a structured program. They asked how quickly they can see visible changes.");
  ...
  const exportLeads = trpc.growth.exportLeads.useQuery(undefined, { enabled: showExport && !!user });
  const sessions = trpc.growth.sessions.useQuery(undefined, { enabled: !!user });
  const uniqueSources = useMemo(() => Array.from(new Set((data?.leads ?? []).map((l) => l.source))), [data?.leads]);

  const filteredLeads = useMemo(() => (data?.leads ?? []).filter((lead) => {
    const matchesSearch = `${lead.name} ${lead.email} ${lead.company ?? ""}`.toLowerCase().includes(query.toLowerCase());
    const matchesStage = stageFilter === "all" || lead.stage === stageFilter;
    const matchesSource = sourceFilter === "all" || lead.source === sourceFilter;
    return matchesSearch && matchesStage && matchesSource;
  }), [data?.leads, query, stageFilter, sourceFilter]);
    ...
    <div className="min-h-screen bg-[#f7f8f4] text-[#172018]">
      <aside className="fixed inset-y-0 left-0 hidden w-[238px] flex-col border-r border-[#e2e7df] bg-[#fbfcf9] px-4 py-5 lg:flex">
        ...
        <nav className="space-y-1">
          ...
          <a href="#leads" className="flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm text-[#6d786e] hover:bg-[#f0f3ed]"><Users className="h-4 w-4" /> Leads <span className="ml-auto rounded-full bg-[#eef1ea] px-2 py-0.5 text-[10px]">{stats?.totalLeads ?? 0}</span></a>
          <a href="/automations" className="flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm text-[#6d786e] hover:bg-[#f0f3ed]"><Zap className="h-4 w-4" /> Automations <span className="ml-auto h-2 w-2 rounded-full bg-[#9fca4d]" /></a>
          ...
          <a href="/chat-history" className="flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm text-[#6d786e] hover:bg-[#f0f3ed]"><MessageSquareText className="h-4 w-4" /> Chat history</a>
        </nav>
        <p className="px-3 mb-2 mt-8 text-[10px] font-semibold uppercase tracking-[0.16em] text-[#9aa49b]">Insights</p>
        <nav className="space-y-1">
          <a href="#activity" className="flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm text-[#6d786e] hover:bg-[#f0f3ed]"><Activity className="h-4 w-4" /> Activity</a>
          <a href="/automations" className="flex items-center gap-3 rounded-xl px-3 py-2.5 text-sm text-[#6d786e] hover:bg-[#f0f3ed]"><Target className="h-4 w-4" /> Playbooks</a>
        </nav>
        <div className="mt-auto rounded-2xl bg-[#173b2d] p-4 text-white">
          ...
          <p className="text-sm font-medium">AI qualification tools</p><p className="mt-1 text-xs leading-relaxed text-[#c3d0c5]">Review qualification suggestions before sending follow-ups.</p>
      ...
      <main className="lg:pl-[238px]">
        <header className="flex items-center justify-between border-b border-[#e2e7df] bg-[#fbfcf9]/80 px-5 py-4 backdrop-blur md:px-9">
          <div><p className="text-xs font-medium text-[#8a978b]">{new Intl.DateTimeFormat(undefined, { weekday: "long", month: "long", day: "numeric", year: "numeric" }).format(new Date())}</p><h1 className="mt-1 text-xl font-semibold tracking-tight">Good morning, {user.name?.split(" ")[0] || "there"} <span className="text-[#9fca4d]">✦</span></h1></div>
        ...
        <div className="px-5 py-7 md:px-9">
          <div className="mb-7 flex flex-col justify-between gap-4 md:flex-row md:items-end"><div><p className="text-sm font-medium text-[#708075]">Acquisition overview</p><h2 className="mt-1 text-3xl font-semibold tracking-[-0.04em]">Your pipeline, at a glance.</h2></div><div className="flex items-center gap-2 text-xs text-[#718073]"><span className="h-2 w-2 rounded-full bg-[#9fca4d]" /> Live workspace <span className="ml-2 rounded-full bg-[#eef4e6] px-2 py-1 text-[#54713d]">Connected</span></div></div>
          <section className="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
            {[{ label: "Total leads", value: stats?.totalLeads ?? 0, detail: "In your workspace", icon: Users, tone: "bg-[#e7f0f9] text-[#52799d]" }, { label: "Qualified", value: stats?.qualified ?? 0, detail: `${stats?.qualificationRate ?? 0}% qualification rate`, icon: Target, tone: "bg-[#eee8fb] text-[#8066aa]" }, { label: "Calls booked", value: stats?.booked ?? 0, detail: `${stats?.bookingRate ?? 0}% of qualified`, icon: CalendarDays, tone: "bg-[#fff1d9] text-[#ad7c30]" }, { label: "Pipeline value", value: `$${((stats?.pipelineValue ?? 0) / 1000).toFixed(1)}k`, detail: "Booked + won opportunities", icon: CircleDollarSign, tone: "bg-[#e5f3dc] text-[#609048]" }].map((item) => <div key={item.label} className="rounded-2xl border border-[#e0e6dc] bg-[#fbfcf9] p-5 shadow-[0_3px_12px_rgba(45,65,42,0.03)]"><div className="flex items-start justify-between"><div><p className="text-xs font-medium text-[#7b887d]">{item.label}</p><p className="mt-2 text-3xl font-semibold tracking-tight">{item.value}</p></div><div className={`flex h-9 w-9 items-center justify-center rounded-xl ${item.tone}`}><item.icon className="h-4 w-4" /></div></div><p className="mt-4 text-[11px] font-medium text-[#6b9860]"><ArrowUpRight className="mr-1 inline h-3 w-3" />{item.detail}</p></div>)}
          </section>

          <section className="mt-6 grid gap-5 xl:grid-cols-[1.35fr_0.65fr]">
            <div className="rounded-2xl border border-[#e0e6dc] bg-[#fbfcf9] p-5 shadow-[0_3px_12px_rgba(45,65,42,0.03)]"><div className="flex items-center justify-between"><div><h3 className="font-semibold">Pipeline flow</h3><p className="mt-1 text-xs text-[#849085]">How leads are moving through your funnel</p></div><button className="rounded-lg border border-[#e1e7df] p-2 text-[#7c897d]"><Filter className="h-3.5 w-3.5" /></button></div><div className="mt-6 space-y-4">{stages.slice(0, 4).map((stage) => { const count = data?.leads.filter((lead) => lead.stage === stage.key).length ?? 0; const percent = data?.leads.length ? Math.round((count / data.leads.length) * 100) : 0; return <div key={stage.key}><div className="mb-2 flex items-center justify-between text-xs"><span className="flex items-center gap-2 font-medium"><span className={`h-2 w-2 rounded-full ${stage.dot}`} />{stage.label}</span><span className="text-[#8a978b]">{count} leads <span className="ml-2 text-[#b2bab2]">{percent}%</span></span></div><div className="h-2 overflow-hidden rounded-full bg-[#edf0ea]"><div className={`h-full rounded-full ${stage.dot} transition-all`} style={{ width: `${percent}%` }} /></div></div> })}</div><div className="mt-6 flex items-center gap-3 rounded-xl bg-[#f1f6eb] px-4 py-3 text-xs text-[#58714e]"><Sparkles className="h-4 w-4 text-[#90b848]" /><span><strong className="font-semibold">Next best action:</strong> {stats?.totalLeads ? "Review qualified leads and invite the strongest fit to book." : "Connect a form or add your first lead to start your pipeline."}</span></div></div>
            <div className="rounded-2xl border border-[#e0e6dc] bg-[#173b2d] p-5 text-white shadow-[0_3px_12px_rgba(23,59,45,0.12)]"><div className="flex items-center justify-between"><div><p className="text-xs font-medium text-[#bdcdbf]">Conversion pulse</p><h3 className="mt-1 text-lg font-semibold">Live pipeline signals</h3></div><div className="rounded-lg bg-white/10 p-2 text-[#d8f27f]"><Activity className="h-4 w-4" /></div></div><div className="mt-8 flex items-end gap-1.5">{stages.slice(0, 4).map((stage) => { const count = data?.leads.filter((lead) => lead.stage === stage.key).length ?? 0; return <div key={stage.key} className={`flex-1 rounded-t ${stage.dot}`} style={{ height: `${Math.max(12, Math.min(94, count * 18))}px` }} />; })}</div><div className="mt-3 flex justify-between text-[10px] text-[#a7b9aa]"><span>Current stages</span><span>{stats?.totalLeads ?? 0} total leads</span></div><div className="mt-6 grid grid-cols-2 gap-3 border-t border-white/10 pt-4"><div><p className="text-2xl font-semibold">{analytics?.booked ?? 0}</p><p className="mt-1 text-[10px] text-[#a7b9aa]">Booked calls</p></div><div><p className="text-2xl font-semibold">{analytics?.won ?? 0}</p><p className="mt-1 text-[10px] text-[#a7b9aa]">Won clients</p></div></div></div>
          </section>

          <section id="insights" className="mt-6 rounded-2xl border border-[#e0e6dc] bg-[#fbfcf9] p-5 shadow-[0_3px_12px_rgba(45,65,42,0.03)]"><div className="flex items-start justify-between gap-4"><div><p className="text-[10px] font-semibold uppercase tracking-[0.16em] text-[#8a978b]">Source insights</p><h3 className="mt-1 font-semibold">Know which sources create conversations.</h3><p className="mt-1 text-xs text-[#849085]">Conversion signals from your live lead pipeline.</p></div><Activity className="h-5 w-5 text-[#83ad4e]" /></div><div className="mt-5 grid grid-cols-2 gap-3 md:grid-cols-5"><div className="rounded-xl bg-[#f1f6eb] p-3"><p className="text-xl font-semibold text-[#173b2d]">{analytics?.replied ?? 0}</p><p className="mt-1 text-[10px] text-[#718073]">Replies</p></div><div className="rounded-xl bg-[#f7f9f5] p-3"><p className="text-xl font-semibold text-[#173b2d]">{analytics?.unsubscribed ?? 0}</p><p className="mt-1 text-[10px] text-[#718073]">Unsubscribed</p></div><div className="rounded-xl bg-[#f7f9f5] p-3"><p className="text-xl font-semibold text-[#173b2d]">{analytics?.total ? Math.round(((analytics.booked / analytics.total) * 100)) : 0}%</p><p className="mt-1 text-[10px] text-[#718073]">Booked rate</p></div><div className="rounded-xl bg-[#f7f9f5] p-3"><p className="text-xl font-semibold text-[#173b2d]">{analytics?.total ? Math.round(((analytics.won / analytics.total) * 100)) : 0}%</p><p className="mt-1 text-[10px] text-[#718073]">Won rate</p></div><div className="col-span-2 rounded-xl bg-[#173b2d] p-3 text-white md:col-span-1"><p className="truncate text-sm font-semibold">{analytics?.bySource?.[0]?.source ?? "No source data"}</p><p className="mt-1 text-[10px] text-[#bdcdbf]">Top lead source</p></div></div></section>

          <section id="leads" className="mt-6 grid gap-5 xl:grid-cols-[1.35fr_0.65fr]">
            <div className="rounded-2xl border border-[#e0e6dc] bg-[#fbfcf9] shadow-[0_3px_12px_rgba(45,65,42,0.03)]"><div className="flex flex-col gap-3 border-b border-[#e8ece6] p-5 md:flex-row md:items-center md:justify-between"><div><h3 className="font-semibold">Recent leads</h3><p className="mt-1 text-xs text-[#849085]">Your highest-priority conversations</p></div><div className="flex items-center gap-2"><div className="relative"><MessageSquareText className="absolute left-2.5 top-2.5 h-3.5 w-3.5 text-[#a1aba0]" /><Input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Search leads…" className="h-8 w-44 rounded-lg border-[#e1e7df] bg-white pl-8 text-xs" /></div><select value={stageFilter} onChange={(e) => setStageFilter(e.target.value)} className="h-8 rounded-lg border border-[#e1e7df] bg-white px-2 text-xs text-[#5e6a60]"><option value="all">All stages</option>{stages.map((s) => <option key={s.key} value={s.key}>{s.label}</option>)}</select><select value={sourceFilter} onChange={(e) => setSourceFilter(e.target.value)} className="h-8 rounded-lg border border-[#e1e7df] bg-white px-2 text-xs text-[#5e6a60]"><option value="all">All sources</option>{uniqueSources.map((src) => <option key={src} value={src}>{src}</option>)}</select><button onClick={() => setShowExport(true)} className="rounded-lg border border-[#e1e7df] px-2.5 py-2 text-[10px] font-medium text-[#5e6a60] hover:bg-[#f0f3ed]">Export</button></div></div><div className="overflow-x-auto"><table className="w-full text-left text-xs"><thead className="bg-[#f7f9f5] text-[10px] uppercase tracking-[0.12em] text-[#9aa49b]"><tr><th className="px-5 py-3 font-medium">Lead</th><th className="px-5 py-3 font-medium">Source</th><th className="px-5 py-3 font-medium">Stage</th><th className="px-5 py-3 font-medium">Score</th><th className="px-5 py-3 font-medium">Activity</th></tr></thead><tbody>{filteredLeads.map((lead) => { const stage = stages.find((item) => item.key === lead.stage) ?? stages[0]; return <tr key={lead.id} className="border-t border-[#edf0eb] hover:bg-[#f8faf6] cursor-pointer" onClick={() => window.location.href = `/leads/${lead.id}`}><td className="whitespace-nowrap px-5 py-3.5"><div className="flex items-center gap-3"><div className="flex h-8 w-8 items-center justify-center rounded-full bg-[#e6eee1] text-[10px] font-semibold text-[#577252]">{initials(lead.name)}</div><div><p className="font-medium text-[#29382d]">{lead.name}</p><p className="mt-0.5 text-[10px] text-[#9aa49b]">{lead.company ?? lead.email}</p></div></div></td><td className="px-5 py-3.5 text-[#728073]">{lead.source}</td><td className="px-5 py-3.5"><span className={`rounded-full px-2 py-1 text-[10px] font-medium ${stage.color}`}>{stage.label}</span></td><td className="px-5 py-3.5"><span className={`font-semibold ${lead.score >= 85 ? "text-[#6c984d]" : "text-[#9b8454]"}`}>{lead.score}</span><span className="text-[#b0b8b0">/100</span></td><td className="whitespace-nowrap px-5 py-3.5 text-[#8b968c]">{relativeTime(lead.lastActivityAt)}</td></tr> })}</tbody></table></div></div>
            <div id="activity" className="rounded-2xl border border-[#e0e6dc] bg-[#fbfcf9] p-5 shadow-[0_3px_12px_rgba(45,65,42,0.03)]"><div className="flex items-center justify-between"><div><h3 className="font-semibold">Activity</h3><p className="mt-1 text-xs text-[#849085]">Latest system signals</p></div><button className="text-[11px] font-medium text-[#6d8e4e]">View all</button></div><div className="mt-5 space-y-5">{(data?.activities ?? []).slice(0, 4).map((item) => <div key={item.id} className="flex gap-3"><div className="mt-0.5 flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-[#eef4e8] text-[#73955c]"><Check className="h-3.5 w-3.5" /></div><div><p className="text-xs font-medium leading-snug text-[#405043]">{item.title}</p><p className="mt-1 text-[10px] leading-relaxed text-[#89948a]">{item.description}</p><p className="mt-1.5 text-[10px] text-[#b0b8b0]">{relativeTime(item.createdAt)}</p></div></div>)}</div><div className="mt-6 border-t border-[#edf0eb] pt-4"><button onClick={() => setShowAi(true)} className="flex w-full items-center justify-center gap-2 rounded-xl bg-[#eef5e2] py-2.5 text-xs font-semibold text-[#52703d] hover:bg-[#e5f0d6]"><Bot className="h-4 w-4" /> Run a qualification check</button></div></div>
          </section>
        </div>
      </main>
      ...
      <Dialog open={showAi} onOpenChange={setShowAi}><DialogContent className="rounded-2xl border-[#dfe7da] bg-[#fbfcf9]"><DialogHeader><DialogTitle className="flex items-center gap-2"><div className="flex h-8 w-8 items-center justify-center rounded-lg bg-[#eaf4d8] text-[#6d9646]"><Bot className="h-4 w-4" /></div> AI qualification check</DialogTitle><DialogDescription>Paste a DM conversation, intake answer, or prospect message. The assistant returns a suggested stage and next step for review.</DialogDescription></DialogHeader><Textarea value={transcript} onChange={(event) => setTranscript(event.target.value)} className="min-h-32" /><Button disabled={qualify.isPending} onClick={() => qualify.mutate({ transcript }, { onSuccess: (result) => setQualification(result), onError: () => toast.error("Could not run qualification") })} className="bg-[#173b2d] hover:bg-[#24533e]"><Send className="mr-2 h-4 w-4" />{qualify.isPending ? "Analyzing…" : "Analyze lead"}</Button>{qualification && <div className="rounded-xl border border-[#dfe8d5] bg-[#f1f7e8] p-4"><div className="flex items-center justify-between"><p className="text-xs font-semibold uppercase tracking-[0.12em] text-[#719052]">Suggested qualification</p><span className="text-2xl font-semibold text-[#416d3b]">{qualification.score}<span className="text-xs font-normal text-[#78906a]"> / 100</span></span></div><p className="mt-3 text-sm font-medium text-[#3e533f]">{qualification.summary}</p><p className="mt-2 text-xs leading-relaxed text-[#718272]"><strong>Suggested action:</strong> {qualification.nextStep}</p></div>}</DialogContent></Dialog>
      <Dialog open={showExport} onOpenChange={setShowExport}><DialogContent className="rounded-2xl border-[#dfe7da] bg-[#fbfcf9]"><DialogHeader><DialogTitle>Export leads</DialogTitle><DialogDescription>Download your leads as a CSV file.</DialogDescription></DialogHeader>{exportLeads.data ? <div className="space-y-4"><p className="text-sm text-[#718073]">{exportLeads.data.count} leads ready to export.</p><Button onClick={() => { const blob = new Blob([exportLeads.data!.csv], { type: "text/csv" }); const url = URL.createObjectURL(blob); const a = document.createElement("a"); a.href = url; a.download = "coachflow-leads.csv"; a.click(); URL.revokeObjectURL(url); toast.success("CSV downloaded"); }} className="w-full bg-[#173b2d] text-[#eff7de]">Download CSV</Button></div> : <p className="text-sm text-[#849085]">Loading…</p>}</DialogContent></Dialog>
    </div>
  );
}
